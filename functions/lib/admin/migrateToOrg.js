...
const parseArgs = () => {
    ...
    const envProjectId = projectId ||
        process.env.GOOGLE_CLOUD_PROJECT ||
        process.env.GCLOUD_PROJECT ||
        process.env.FIREBASE_PROJECT_ID;
    return { orgId, dryRun, projectId: envProjectId };
};
const readProjectIdFromFirebaseConfig = () => {
...
const ensureFirestore = (projectId) => {
...
const resolveUid = (docId, data) => {
    const candidate = (typeof data.uid === "string" && data.uid) ||
...
const isAdminUser = (data) => {
    ...
    const isAdmin = data.isAdmin === true || data.admin === true || roleRaw === "admin";
    return isAdmin;
};
...
const normalizeInvoiceData = (data) => {
    const payload = { ...data };
    const totalsRaw = typeof payload.totals === "object" && payload.totals !== null ? payload.totals : {};
...
const copyCollection = async ({ source, target, dryRun, transform, projectId }) => {
    const db = ensureFirestore(projectId);
    let lastDoc = null;
    let totalCopied = 0;
    while (true) {
        let queryRef = db.collection(source).orderBy(firestore_1.FieldPath.documentId()).limit(PAGE_SIZE);
        if (lastDoc) {
...
const loadAdminUsers = async (projectId) => {
    const db = ensureFirestore(projectId);
    const admins = [];
    const seen = new Set();
    let lastDoc = null;
    let skipped = 0;
    while (true) {
        let queryRef = db.collection("users").orderBy(firestore_1.FieldPath.documentId()).limit(PAGE_SIZE);
        if (lastDoc) {
            queryRef = queryRef.startAfter(lastDoc);
        }
        const snapshot = await queryRef.get();
...
const ensureOrgDocuments = async (orgId, dryRun, projectId) => {
    const db = ensureFirestore(projectId);
...
const writeMembers = async (orgId, admins, dryRun, projectId) => {
    const db = ensureFirestore(projectId);
...
const markMigrated = async (orgId, dryRun, projectId) => {
    if (dryRun) {
        return;
    }
    const db = ensureFirestore(projectId);
